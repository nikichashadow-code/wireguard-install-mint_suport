...
function checkOS() {
    ...
    if [[ ${OS} == "debian" || ${OS} == "raspbian" ]]; then
        ...
        if [[ ${RELEASE_YEAR} -lt 18 ]]; then
            echo "Your version of Ubuntu/Linux Mint (${VERSION_ID}) is not supported. Please use Ubuntu 18.04 / Linux Mint 19 or later"
            exit 1
        fi
    elif [[ ${OS} == "fedora" ]]; then
        ...
        echo "Looks like you aren't running this installer on a Debian, Ubuntu, Linux Mint, Fedora, CentOS, AlmaLinux, Oracle or Arch Linux system"
        exit 1
    fi
}
...
function installWireGuard() {
    ...
    if [[ ${OS} == 'ubuntu' || ${OS} == 'linuxmint' ]] || [[ ${OS} == 'debian' && ${VERSION_ID} -gt 10 ]]; then
        apt-get update
